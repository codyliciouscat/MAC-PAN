...
--------------------CREATE PATH--------------------
=================================================== */
void create_path(const Vertex source, const Vertex to, PathTable & path_table, map<Vertex, Vertex> & predecessor)
{
  ...
  else
  {
    if(path_table.table[source][to].size() == 0)
      create_path(source, predecessor[to], path_table, predecessor);
...
void dijkstra(const Vertex source, const vector<Edge> edges, const vector<Vertex> vertices, World & graph, PathTable & path_table)
{
  ...
  for(int i = 0; i < vertices.size(); i++)
  {
    create_path(source, vertices[i], path_table, predecessor);
  }
  //path_table.print();
//}
